...
-- Keep SECURITY DEFINER RLS helpers outside the exposed public schema.
...
CREATE OR REPLACE FUNCTION private.ops_is_staff()
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path=''
AS $$ SELECT EXISTS (SELECT 1 FROM public.ops_staff WHERE id=(SELECT auth.uid()) AND active); $$;
CREATE OR REPLACE FUNCTION private.ops_is_super_admin()
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path=''
AS $$ SELECT EXISTS (SELECT 1 FROM public.ops_staff WHERE id=(SELECT auth.uid()) AND active AND role='Super admin'); $$;
CREATE OR REPLACE FUNCTION private.ops_is_admin()
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path=''
AS $$ SELECT EXISTS (SELECT 1 FROM public.ops_staff WHERE id=(SELECT auth.uid()) AND active AND role IN ('Super admin','Center admin')); $$;
CREATE OR REPLACE FUNCTION private.ops_scope()
RETURNS text LANGUAGE sql STABLE SECURITY DEFINER SET search_path=''
AS $$ SELECT scope FROM public.ops_staff WHERE id=(SELECT auth.uid()) AND active LIMIT 1; $$;
CREATE OR REPLACE FUNCTION private.ops_can_access_center(p_center_id uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path=''
AS $$ SELECT private.ops_is_super_admin() OR private.ops_scope()='All centers' OR EXISTS (SELECT 1 FROM public.ops_centers c WHERE c.id=p_center_id AND c.name=private.ops_scope()); $$;
